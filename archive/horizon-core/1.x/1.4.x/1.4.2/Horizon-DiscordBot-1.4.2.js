// git: ATTENTION : Cette version (1.4.2-10) a été laissée dans un état où elle peut être instable...
//
//  _   _            _
...
//
// Horizon Discord Bot version 1.4.2
//
...
client.on('message', function (message) {
    ...
    if (message.content.startsWith('><d')) {
        ...
        if (message.author.username == "Minteck | ルカリオ" || message.author.username == "Horizon.Data") {
			message.delete()
			lstmsg = message
			loginfo = "Distribution des informations de déboggage sur le salon #" + message.channel.name + " du serveur " + message.guild.name
			showLog();
			let system = os.type()
			let release = os.release()
			let cpuarch = os.arch()
...
client.on('message', function (message) {
    ...
    if (message.content.startsWith('><md')) {
        ...
        if (message.author.username == "Minteck | ルカリオ" || message.author.username == "Horizon.Data") {
			...
			showLog();
			if (client.guildMember.voiceChannel === null || client.guildMember.voiceChannel === 'undefined') {
			message.channel.send("```\nInformations de déboggage de la musique pour ce petit Gardevoir qui me les a demandées :\n\n" + "               Connexion vocale : " + true + "\n\nInformations du salon vocal :" + "                 Salon courrant : " + client.user.voiceChannel.name + "                  Salon complet : " + client.user.voiceChannel.full + "           Débit du salon vocal : " + client.user.voiceChannel.bitrate + "       Catégorie du salon vocal : " + client.user.voiceChannel.parentID + " Limite d'utilisateurs du salon : " + client.user.voiceChannel.position + "      Peut parler dans le salon : " + client.user.voiceChannel.speakable + "\n\nInformations sur la connexion au salon :" + "        Horizon transmet du son : " + client.user.voiceChannel.connection.speaking + "           Êtat de la connexion : " + client.user.voiceChannel.connection.status + "\n\nInformations concernant la transmission audio sortante :" + "                   (actuellement indisponible)\n```")
            }else{
...
client.on('message', message => {
  ...
  if (message.content === '><mj') {
    ...
    if (message.member.voiceChannel) {
		if (client.guildMember.voiceChannel === null || client.guildMember.voiceChannel === 'undefined') {
          message.author.send(":no_entry: Je suis déjà connecté au salon vocal **" + client.user.voiceChannel.name + '**. Utilisez `><ml` pour le quitter...');
    } else {
...
client.on('message', message => {
  ...
  if (message.content === '><ml') {
	  ...
	  if (message.author.username == "Minteck | ルカリオ" || message.author.username == "Horizon.Data") {
		  message.delete()
    if (client.guildMember.voiceChannel === null || client.guildMember.voiceChannel === 'undefined') {
          message.author.send(":white_check_mark: J'ai quitté le salon vocal **" + client.user.voiceChannel.name + '**...');
        client.user.voiceChannel.leave()
    } else {
