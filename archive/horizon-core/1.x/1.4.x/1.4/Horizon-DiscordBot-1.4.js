...
//
// Horizon Discord Bot version 1.4
//
  ...
  if (message.content === '><mj') {
    ...
    if (message.member.voiceChannel) {
      message.member.voiceChannel.join()
        .then(connection => {
          message.reply(":white_check_mark: J'ai rejoint le salon vocal **" + message.member.voiceChannel.name + '**...');
        })
        .catch(console.log);
    } else {
      message.reply(":no_entry: Vous n'êtes pas connecté dans un salon vocal. Vous devez en rejoindre un pour que je puisse me connecter...");
    }
  }else{
				message.channel.send("```\n403: Accès refusé\nCette action sera signalée\n```")
                loginfo = "Rejet d'accès à l'utilisateur @" + message.author.username + "#" + message.author.tag + " (" + message.author.id + ") depuis le serveur " + message.guild.name + " (#" + message.channel.name + ")" + " | " + message.content
...
client.on('message', message => {
  ...
  if (message.content === '><ml') {
    ...
    if (client.user.voiceChannel) {
          message.reply(":white_check_mark: Je quitterai le salon vocal **" + client.user.voiceChannel.name + '** dans quelques instants...');
        setTimeout(requestRestart, 5000)
    } else {
      message.reply(":no_entry: **Horizon** n'est pas encore connecté à un salon vocal. Utilisez la commande `><mj` pour en rejoindre un...");
    }
  }else{
				message.channel.send("```\n403: Accès refusé\nCette action sera signalée\n```")
                loginfo = "Rejet d'accès à l'utilisateur @" + message.author.username + "#" + message.author.tag + " (" + message.author.id + ") depuis le serveur " + message.guild.name + " (#" + message.channel.name + ")" + " | " + message.content
